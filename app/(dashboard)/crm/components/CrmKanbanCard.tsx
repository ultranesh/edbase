...
export default function CrmKanbanCard({ lead, onCardClick, formatAmount, isOverlay, waUnreadCount = 0, socialUnreadCount = 0 }: CrmKanbanCardProps) {
  ...
  return (
    <div
      ...
      className={`bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-2.5 shadow-sm hover:shadow-md transition-shadow cursor-grab active:cursor-grabbing select-none ${
        isDragging ? 'opacity-30' : ''
      } ${isOverlay ? 'shadow-xl' : ''}`}
    >
      {/* Name + Phone row */}
      <div className="flex items-center gap-2 mb-1.5">
        <div className="h-7 w-7 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center shrink-0 relative">
          <span className="text-[10px] font-medium text-blue-600 dark:text-blue-400">{initials}</span>
          {(waUnreadCount > 0 || socialUnreadCount > 0) && (
            <span className="absolute -top-0.5 -right-0.5 min-w-[14px] h-3.5 flex items-center justify-center rounded-full bg-red-500 text-white text-[8px] font-bold px-0.5 shadow-sm">
      ...
      <div className="flex items-center justify-between gap-1.5">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {lead.source && (
            <span className="text-[9px] px-1 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-gray-500 dark:text-gray-400 truncate max-w-[50px]">
              {lead.source}
            </span>
          )}
          {lead.language && (
            <span className="text-[9px] px-1 py-0.5 bg-blue-50 dark:bg-blue-900/20 rounded text-blue-600 dark:text-blue-400 font-medium">
              {lead.language}
            </span>
          )}
