      ...
      <div className="px-4 py-3 border-b border-gray-200 dark:border-[#222e35] flex items-center gap-2 bg-white dark:bg-[#202c33]">
        ...
        {leadPhone && parentPhone && (
          <div className="flex gap-1 ml-2">
            <button
              onClick={() => {
                const conv = conversations.find(c => normalizePhone(c.contactPhone) === normalizePhone(leadPhone));
                if (conv) { setActiveConversation(conv); setIsBlocked(!!conv.isBlocked); }
                else { setInitPhone(leadPhone); setActiveConversation(null); }
              }}
              className={`text-xs px-2.5 py-1 rounded-lg transition-colors ${
                (activeConversation && normalizePhone(activeConversation.contactPhone) === normalizePhone(leadPhone)) || (!activeConversation && initPhone === leadPhone)
            ...
            <button
              onClick={() => {
                const conv = conversations.find(c => normalizePhone(c.contactPhone) === normalizePhone(parentPhone));
                if (conv) { setActiveConversation(conv); setIsBlocked(!!conv.isBlocked); }
                else { setInitPhone(parentPhone); setActiveConversation(null); }
              }}
              className={`text-xs px-2.5 py-1 rounded-lg transition-colors ${
                (activeConversation && normalizePhone(activeConversation.contactPhone) === normalizePhone(parentPhone)) || (!activeConversation && initPhone === parentPhone)
                  ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 font-medium'
                  : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600'
              }`}
            >
      ...
      {loading ? (
        ...
        <>
          ...
          <div
            ...
            className="flex-1 overflow-y-auto px-3 py-2 space-y-1 bg-[#efeae2] dark:bg-[#0b141a] relative min-h-0"
